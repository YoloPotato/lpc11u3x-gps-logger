...
#if defined(CHIP_LPC11AXX) || defined(CHIP_LPC11EXX) || defined(CHIP_LPC11UXX)
/* EEPROM single device generation bit */
#define FMC_EEPROM_DEVSEL		(1UL << 30)
/* EEPROM signature start and busy status bit */
#define FMC_EEPROM_STRTBIST		(1UL << 31)

/**
 * @brief	Start computation of a signature for a EEPROM memory range
 * @param	start	: Starting EEPROM address for computation, must be 16-bit aligned
 * @param	stop	: Ending EEPROM address for computation, must be 16-bit aligned
 * @return	Nothing
...
{
	LPC_FMC->EEMSSTART = (start >> 4);
	LPC_FMC->EEMSSTOP = (stop >> 4) | FMC_EEPROM_STRTBIST;
}

/**
 * @brief	Start computation of a signature for a FLASH memory address and block count
 * @param	start	: Starting EEPROM address for computation, must be 16-bit aligned
 * @param	words	: Number of 2 byte blocks used for computation
 * @return	Nothing
 * @note	Only bits 13..1 are used for the EEPROM signature computation.
 *			Use the Chip_FMC_IsEEPROMSignatureBusy() function to determine when the
 *			signature computation operation is complete.
 */
STATIC INLINE void Chip_FMC_ComputeEEPROMSignatureBlocks(uint32_t start, uint32_t words)
{
	Chip_FMC_ComputeSignature(start, (start + (words * 2)));
}
...
{
	return (bool) ((LPC_FMC->EEMSSTOP & FMC_EEPROM_STRTBIST) != 0);
}
 ...
 */
STATIC INLINE uint32_t Chip_FMC_GetEEPROMDataSignature(int index)
{
	return LPC_FMC->EEMSSIG[index] & 0xFFFF;
}
 ...
 */
STATIC INLINE uint32_t Chip_FMC_GetEEPROMParitySignature(int index)
{
	return LPC_FMC->EEMSSIG[index] >> 16;
}
