...
#if defined(CHIP_LPC11CXX) || defined(CHIP_LPC11EXX) || defined(CHIP_LPC11UXX) ||	\
...
void Chip_PMU_SleepState(LPC_PMU_T *pPMU)
{
	...
	reg = pPMU->PCON & ~PMU_PCON_PM;
	pPMU->PCON = reg | PMU_PCON_PM_SLEEP;
#elif defined(CHIP_LPC11CXX) || defined(CHIP_LPC1125)
...
void Chip_PMU_DeepSleepState(LPC_PMU_T *pPMU)
{
	...
	reg = pPMU->PCON & ~PMU_PCON_PM;
	pPMU->PCON = reg | PMU_PCON_PM_DEEPSLEEP;
#elif defined(CHIP_LPC11CXX) || defined(CHIP_LPC1125)
	pPMU->PCON &= ~PMU_PCON_DPDEN;
#endif
	Chip_PMU_ClearSleepFlags(LPC_PMU, PMU_PCON_SLEEPFLAG | PMU_PCON_DPDFLAG);
...
void Chip_PMU_PowerDownState(LPC_PMU_T *pPMU)
{
	...
	reg = pPMU->PCON & ~PMU_PCON_PM;
	pPMU->PCON = reg | PMU_PCON_PM_POWERDOWN;
	Chip_PMU_ClearSleepFlags(LPC_PMU, PMU_PCON_SLEEPFLAG | PMU_PCON_DPDFLAG);
	SCB->SCR |= (1UL << SCB_SCR_SLEEPDEEP_Pos);
...
#endif /* defined(CHIP_LPC11EXX) || defined(CHIP_LPC11UXX) */

/* Enter MCU Deep Power down mode */
void Chip_PMU_DeepPowerDownState(LPC_PMU_T *pPMU)
{
#if defined(CHIP_LPC11EXX) || defined(CHIP_LPC11UXX)
	// Note that we are force clearing NODPD bit
...
#elif defined(CHIP_LPC11AXX) || defined(CHIP_LPC11XXLV) || defined(CHIP_LPC110X)

/* Enter MCU Sleep mode */
void Chip_PMU_SleepState(LPC_PMU_T *pPMU)
{
#if defined(CHIP_LPC110X)
	Chip_PMU_ClearSleepFlags(LPC_PMU, PMU_PCON_SLEEPFLAG);
#endif
	SCB->SCR &= ~(1UL << SCB_SCR_SLEEPDEEP_Pos);
...
#if defined(CHIP_LPC11XXLV) || defined(CHIP_LPC110X)
/* Enter MCU Deep Sleep mode */
void Chip_PMU_DeepSleepState(LPC_PMU_T *pPMU)
{
#if defined(CHIP_LPC110X)
...
#endif /* defined(CHIP_LPC11CXX) || defined(CHIP_LPC11EXX) || ... */
